...
void task()
{
	...
	while(t--){
		...
		//cout << "K = " << 0 << " sum[" << 0 << "] " << sum[0] << "\n";
		//cout << "K = " << 1 << " sum[" << 1 << "] " << sum[1] << "\n";
		for(int K=2; K<n; K++){
			if(s[K]=='+') sum[K] = sum[K-2] + 1;
			else sum[K] = sum[K-2] - 1;
		...
		while(q--){
			...
			if(l>1) ans = sum[r-abs((r%2)-(l%2))] - sum[l-2];
			else ans = sum[r-abs((r%2)-(l%2))];
			
			l++;
			if(r>=l && l>1) ans -= sum[r-abs((r%2)-(l%2))] - sum[l-2];
			else if(r>=l) ans -= sum[r-abs((r%2)-(l%2))];
			
			cout << (abs(ans)==0?0:abs(ans)%2==0?2:1) << "\n";
		}
	}
}
