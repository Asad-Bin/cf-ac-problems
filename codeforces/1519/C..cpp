...
void task()
{
	...
	while(t--){
		...
		for(int K=1; K<=n; K++) sort(ara[K].rbegin(), ara[K].rend());
		for(int K=1; K<=n; K++){
			for(int L=1; L<sz(ara[K]); L++) ara[K][L] += ara[K][L-1];
		}
		
		dp.clear();
		for(int K=1; K<=n; K++){
			//sum = 0;
			for(int L=0; L<sz(ara[K]); L++){
				dp[L+1] += ara[K][((int)sz(ara[K])/(L+1))*(L+1)-1];
				
				//sum += ara[K][L];
				//for(int M=1; M*M<=L+1; M++){
					//if((L+1)%M==0){
						//dp[M] += ara[K][(L];
						//if(M!=(L+1)/M) dp[(L+1)/M] += ara[K][L];
					//}
				//}
			}
		}
