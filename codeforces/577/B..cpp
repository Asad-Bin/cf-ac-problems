...
int ara[1000006];
int n, m;
vector<bitset<1001> > dp(1000001);
bool calc(int at, int sum)
{
	//cout << at << ' ' << ara[at] << ' ' << sum << "\n";
	
	if(sum != 0 && (sum+ara[at])%m == 0) return 1;
	
	if(at == n) return 0;
	
	if(!dp[at][sum]) return 0;
	
	bool ok = calc(at+1, (sum+ara[at])%m);
	
	//cout << at << ' ' << ara[at] << ' ' << sum << ' ' << ok << "\n";
	if(ok) return 1;
...
void task()
{
	...
	//for(int K=1; K<=5; K++) cout << dp[K][K] << ' ';
	//cout << "\n";
	
	cin >> n >> m;
	for(int K=0; K<n; K++){
		cin >> ara[K];
		//for(int L=1; L<=10; L++) cout << dp[K][L] << ' ';
		//cout << "\n";
		dp[K].set();
	}
