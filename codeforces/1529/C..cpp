...
const int MX = 1e5;
vector<pii> node(MX+5);
vector<vi> ara(MX+2);
bool vis[MX+5];

pair<pll, pll> dfs(int u)
{
	vis[u] = true;
	...
	for(int K=0; K<sz(ara[u]); K++){
		if(!vis[ara[u][K]]){
			x = dfs(ara[u][K]);
			lo += max(x.fi.se+abs(x.fi.fi-node[u].fi), x.se.se+abs(x.se.fi-node[u].fi));
			hi += max(x.fi.se+abs(x.fi.fi-node[u].se), x.se.se+abs(x.se.fi-node[u].se));
		}
	}
	
	//cout << u << ' ' << node[u].fi << ' ' << lo << ' ' << node[u].se << ' ' << hi << "\n";
	return pair<pll, pll> {{node[u].fi, lo}, {node[u].se, hi}};
}
void task()
{
	// Code here.. ..
	...
	while(t--){
		...
		for(int K=1; K<=n; K++){
			cin >> node[K].fi >> node[K].se;
			if(node[K].fi > node[K].se) swap(node[K].fi, node[K].se);
		}
		...
		pair<pll, pll> ans = dfs(1);
		
		cout << max(ans.fi.se, ans.se.se) << "\n";
	}
}
