...
void task()
{
	...
	while(t--){
		...
		for(auto x:v){
			if(x.fi == 0){
				int l = min(m, x.se-x.fi+1);
				for(int K=1, L=x.se; K<=l; K++, L--) s[L] = '1';
				
				continue;
			}
			if(x.se == n-1){
				int l = min(m, x.se-x.fi+1);
				for(int K=1, L=x.fi; K<=l; K++, L++) s[L] = '1';
				
				continue;
			}
			
			int l = min(m, (x.se-x.fi+1)/2);
			for(int K=1, L=x.fi; K<=l; K++, L++){
				s[L]= '1';
			}
			for(int K=1, L=x.se; K<=l; K++, L--){
				s[L] = '1';
			}
		}
