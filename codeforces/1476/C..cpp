...
void task()
{
	...
	ll t, n, c[100005], a[100005], b[100005];
	ll ara[100005];
	...
	while(t--){
		...
		if(a[1]!=b[1]) ara[1] = c[1]-1LL + 2LL + abs(a[1]-b[1]);
		else ara[1] = c[1]-1LL + 2LL;
		ans = max(ans, ara[1]);
		
		for(int K=2; K<n; K++){
			if(a[K]!=b[K]) ara[K] = max(c[K]-1LL + 2LL + abs(a[K]-b[K]), ara[K-1] + c[K]-1LL + 2LL - abs(a[K]-b[K]));
			else ara[K] = c[K]-1LL + 2LL;
