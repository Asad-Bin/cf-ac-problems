...
ll dp[505][505][50];
ll calc(int K, int L, int stp)
{
	if(stp==0){
		//if(K!=x || L!=y) return MXI;
		return 0;
	}
	if(dp[K][L][stp]!=-1) return dp[K][L][stp];
	
	ll ans = MXI;
	if(K+1<n) ans = calc(K+1, L, stp-1)+b[K][L];
	if(K>0) ans = min(ans, calc(K-1, L, stp-1)+b[K-1][L]);
	if(L+1<m) ans = min(ans, calc(K, L+1, stp-1)+a[K][L]);
	if(L>0) ans = min(ans, calc(K, L-1, stp-1)+a[K][L-1]);
	
	return dp[K][L][stp] = ans;
}
void task()
{
	// Code here.. ..
	...
	for(int K=0; K<n; K++){
		for(int L=0; L<m-1; L++) cin >> a[K][L];
	}
	for(int K=0; K<n-1; K++){
		for(int L=0; L<m; L++) cin >> b[K][L];
	}
